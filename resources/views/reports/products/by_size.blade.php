<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">แสดงรายละเอียดตาม Size</h2>
    </x-slot>

    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg p-6">
            <form method="GET" action="{{ route('reports.products.by-size') }}" class="flex flex-wrap gap-4 mb-6 items-end">
                <div class="flex flex-col">
                    <label class="mb-1 font-medium">Size</label>
                    <select name="size" class="border rounded p-2 w-48">
            ...
            @if($grouped->count())
                @foreach($grouped as $sizeKey => $products)
                    <div class="mb-6">
                        <h3 class="text-lg font-bold bg-blue-100 text-blue-800 px-4 py-2 rounded-t">Size: {{ $sizeKey ?: '-' }}</h3>
