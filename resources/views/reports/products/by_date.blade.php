<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">แสดงสินค้าตามวันเข้าคลัง</h2>
    </x-slot>

    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg p-6">
            <form method="GET" action="{{ route('reports.products.by-date') }}" class="flex flex-wrap gap-4 mb-6 items-end">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full border border-gray-300">
                        <thead class="bg-gray-100">
                            <tr>
                                <th class="border px-3 py-2">วันที่เข้าคลัง</th>
                            ...
                            @foreach($transactions as $t)
                                @foreach($t->items as $i)
                                    <tr>
                                        ...
                                        <td class="border px-3 py-2 text-center">{{ $i->product_id }}</td>
                                        <td class="border px-3 py-2">{{ $i->product->name ?? '-' }}</td>
                                        <td class="border px-3 py-2">{{ $i->product->category->category_name ?? '-' }}</td>
