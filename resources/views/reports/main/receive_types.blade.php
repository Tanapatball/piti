<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">รายงาน - ประเภทจัดเก็บ</h2>
    </x-slot>

    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg p-6">
            <form method="GET" action="{{ route('reports.main.receive-types') }}" class="flex flex-wrap gap-4 mb-6 items-end">
            ...
            @if($receiveTypes->count())
                <div class="overflow-x-auto"><table class="min-w-full border border-gray-300">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="border px-3 py-2">ลำดับ</th>
                            <th class="border px-3 py-2">รหัสประเภท</th>
                            <th class="border px-3 py-2">ชื่อประเภท</th>
                            <th class="border px-3 py-2">จำนวนธุรกรรม</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($receiveTypes as $index => $rt)
                            <tr>
                                ...
                                <td class="border px-3 py-2 text-center">{{ $rt->receive_type_id }}</td>
                                <td class="border px-3 py-2">{{ $rt->name }}</td>
                                <td class="border px-3 py-2 text-center">{{ $rt->transactions_count }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table></div>
