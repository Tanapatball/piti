<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">ประเภทการรับเข้า</h2>
    </x-slot>

    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <form method="GET" action="{{ route('receive-types.index') }}" class="flex flex-col sm:flex-row gap-2 flex-1">
                ...
                @if(request('search'))
                <a href="{{ route('receive-types.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium text-center">ล้าง</a>
                @endif
            </form>
            <a href="{{ route('receive-types.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg shadow text-sm text-center">
                เพิ่มประเภทการรับเข้า
            </a>
        </div>

        @if(request('search'))
        <div class="mb-3 text-sm text-gray-500">
            พบ {{ $receiveTypes->count() }} รายการ | ค้นหา: "{{ request('search') }}"
        </div>
        @endif
            ...
            <div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $type->receive_type_id }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $type->name }}</td>
                        <td class="px-6 py-4 text-sm font-medium flex gap-2">
                            <a href="{{ route('receive-types.edit', $type) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            <form action="{{ route('receive-types.destroy', $type) }}" method="POST" onsubmit="return confirm('ลบรายการนี้?')">
                                @csrf
                                @method('DELETE')
